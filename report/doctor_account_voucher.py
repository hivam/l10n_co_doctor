...
class doctor_account_voucher(report_sxw.rml_parse):
    ...
    def base(self, partner_id, fecha, pago_paciente):
        ...
        cuenta_ids = [] 
        fecha_nueva = datetime.strptime(fecha, '%d-%m-%Y').strftime('%d-%m-%Y')
        cuenta_id = cuenta.search(self.cr, self.uid, [('patient_id', '=', paciente_id[0]), ('date_invoice', '=', fecha_nueva), ('amount_patient', '=', pago_paciente)], context=context)
