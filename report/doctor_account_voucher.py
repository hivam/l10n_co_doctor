...
class doctor_account_voucher(report_sxw.rml_parse):
    ...
    def base(self, partner_id, fecha, pago_paciente):
        ...
        cuenta_ids = [] 
        fecha_nueva = datetime.strptime(fecha, '%d-%m-%Y 00:00:00')
        fecha_nuevaDos = datetime.strptime(fecha, '%d-%m-%Y 23:59:59')
        cuenta_id = cuenta.search(self.cr, self.uid, [('patient_id', '=', paciente_id[0]), ('date_invoice', '>=', fecha_nueva), ('date_invoice', '<=', fecha_nuevaDos), ('amount_patient', '=', pago_paciente)], context=context)
