...
class doctor_account_voucher(report_sxw.rml_parse):
    ...
    def base(self, partner_id, fecha, pago_paciente):
        ...
        cuenta_ids = [] 
        fecha_inicial = datetime.strptime(fecha, "%d-%m-%Y 00:00:00")
        fecha_fin = datetime.strptime(fecha, "%d-%m-%Y 23:59:59")
        cuenta_id = cuenta.search(self.cr, self.uid, [('patient_id', '=', paciente_id[0]), ('date_invoice', '>=', fecha_inicial), ('date_invoice', '<=', fecha_fin), ('amount_patient', '=', pago_paciente)], context=context)
