...
class doctor_patient_co(models.Model):
    ...
    #nc_paciente = fields.Text(compute='_get_nc', type="text", store=False,
    #                              readonly=True)
    ...
    unidad_edad_calculada = fields.Selection(compute='_get_unidad_edad', type="selection", method=True,
    ...
    insurer_prepagada_id =  fields.Many2one('doctor.insurer', "Aseguradora", required=False,
                                            domain="[('tipousuario_id.name','=', 'Otro')]")
    ...
    neighborhood_id =  fields.Many2one('res.country.state.city.neighborhood', 'Barrio', required=False,
                                      domain="[('country_id','=',country_id),('state_id','=',state_id), ('city_id','=',city_id)]")
    ...
    programa_id =  fields.Many2one('doctor.programa_academico', 'Programa Academico', required=False,
                                  domain="[('nivel_estudio','=', (nivel_educativo))]")
    ...
    localidad_otros_paises_id = fields.Many2one('doctor.localidad.country', u'Localidad/Ciudad',
                                                domain="[('country_id','=',nacimiento_country_id)]")
    ...
    def _get_nc(self):
        res = {}
        modelo_atencion = self.env['doctor.attentions']
        nc = ""
        for datos in self.browse([]):
            doctor_id = self.env['doctor.professional'].search([('user_id','=',self._uid)])
            notas_ids =modelo_atencion.search([('patient_id', '=', datos.id),
                                                    ('professional_id', 'in', doctor_id),
                                                    ('notas_confidenciales', '!=', None)])
    ...
    def create(self,vals):
        ...
        if not vals['es_profesionalsalud']:
            id_tercero = self.env['res.partner'].create( {'ref': vals['ref'], 'tdoc': vals['tdoc'],
                                                                       'middlename': vals['middlename'] or '',
                                                                       'surname': vals['surname'] or '',
                                                                       'lastname': vals['lastname'],
                                                                       'firtsname': vals['firstname'],
                                                                       'name': vals['name'], 'image': vals['photo'],
                                                                       'city_id': vals['city_id'],
                                                                       'state_id': vals['state_id'],
                                                                       'street': vals['street'],
                                                                       'phone': vals['telefono'],
                                                                       'mobile': vals['movil'], 'email': vals['email'],
                                                                       'es_paciente': True,
                                                                       'es_profesional_salud': False})
            vals.update(
                {'patient': id_tercero.id })  # una vez creamos el tercero podemos añadir el partner al campo patient.
        else:
            partner_id = self.env['res.partner'].search([('ref', '=', vals['ref'])])
            self.env['res.partner'].write(partner_id, {'es_paciente': True})
    ...
    #def write(self, vals):
    #    vals['ver_nc'] = False
    #    return super(doctor_patient_co,self).write(vals)
    ...
    def _check_unique_ident(self):
        for record in self.browse([]):
            ref_ids = self.search([('ref', '=', record.ref), ('id', '<>', record.id)])
            if ref_ids:
                return False
        return True
    ...
    def _check_seleccion(self):
        if self.prepagada_predeterminada and self.eps_predeterminada:
            return False
        return True
...
class doctor_dependencia(models.Model):
...
class doctor_programa_academico(models.Model):
...
class doctor_patient_co_estadocivil(models.Model):
...
class doctor_patient_co_parentesco(models.Model):
...
class doctor_patient_co_ocupacion(models.Model):
...
class doctor_appointment_co(models.Model):
    ...
    #ref = fields.related('patient_id', 'ref', type=" Char", relation="doctor.patient", string=u'Nº de identificación',
    #                       required=True, readonly=True)
    #phone = fields.related('patient_id', 'telefono', type=" Char", relation="doctor.patient", string=u'Teléfono',
    ...
    ambito = fields.Selection(ambito, u'Ámbito', default=1)
    finalidad = fields.Selection(finalidad, 'Finalidad',default=1)
    ...
    def fields_view_get(self, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(doctor_appointment_co, self).fields_view_get(view_id=view_id, view_type=view_type,  toolbar=toolbar, submenu=submenu)
...
class doctor_appointment_type(models.Model):
    ...
    procedures_id = fields.One2many('doctor.appointment.type_procedures', 'appointment_type_id', 'Procedimientos en Salud',
                                     ondelete='restrict')
    modulos_id = fields.Many2one('ir.module.module', 'Historia asociada',
                               domain="['|', ('author','=','TIX SAS'), '|',('author','=','Proyecto Evoluzion'), '|',('author','=','Diego ivan c'), ('author','=','PROYECTO EVOLUZION'), ]")

doctor_appointment_type()

class doctor_appointment_type_procedures(models.Model):
...
class doctor_attentions_co(models.Model):
    ...
    complicacion_eventoadverso = fields.Selection([('01', 'Ninguno'),
                                                    ...
                                                    ('05', 'Otro')
                                                    ], u'Complicación o Evento Adverso', states={'closed':[('readonly',True)]},default='01')
    ...
    inv = fields.Boolean(compute='_get_creador', type="boolean", store= False,
                            readonly=True, method=True, string='inv', default=True)
    ...
    inv_boton_edad = fields.Boolean(compute='_get_edad', type="boolean", store= False,
    ...
    paciente_identificacion = fields.Char(compute='_get_ref', fnct_inv=_set_ref , type="Char", store= False,
                            string=u'Identificación', required=True)
    paciente_edad_atencion = fields.Integer(compute='_get_edad_paciente', type='integer', store=False, required=True, readonly=True, string='Edad Actual',)
    paciente_unidad_edad = fields.Selection(compute='_get_unidad_edad_paciente', type='selection', selection=[('1', u'Años'), ('2', 'Meses'), ('3', 'Dias'),], string='Unidad medida edad',
    ...
    paciente_primer_nombre = fields.Char(compute='_get_primer_nombre', fnct_inv=_set_primer_nombre, type=" Char", store= False,
                            string=u'Primer Nombre', required=True)
    paciente_segundo_nombre = fields.Char(compute='_get_segundo_nombre', fnct_inv=_set_segundo_nombre, type=" Char", store= False,
                            string=u'Segundo Nombre',)
    paciente_primer_apellido = fields.Char(compute='_get_primer_apellido', fnct_inv=_set_primer_apellido, type=" Char", store= False,
                            string=u'Primer Apellido', required=True)
    paciente_segundo_apellido = fields.Char(compute='_get_segundo_apellido', fnct_inv=_set_segundo_apellido, type=" Char", store= False,
                            string=u'Segundo Apellido',)
    paciente_tdoc = fields.Selection(compute='_get_tdoc', fnct_inv=_set_tdoc, type='selection', selection=[('11','Registro civil'), ('12','Tarjeta de identidad'),
                              ('13',u'Cédula de ciudadanía'), ('21',u'Cédula de extranjería'), ('41','Pasaporte'),
    ...
    paciente_sexo = fields.Selection(compute='_get_sexo',fnct_inv=_set_sexo, type='selection', selection=[('m', 'Masculino'), ('f', 'Femenino'),], string='Sexo',
                                store=False, required=True, readonly=True)

    paciente_ocupacion_actual = fields.Char(compute='_get_ocupacion', fnct_inv=_set_ocupacion , type=" Char", store= False,
                            string=u'Ocupacion')
    paciente_street = fields.Char(compute='_get_direccion', fnct_inv=_set_direccion , type=" Char", store= False,
                            string=u'Dirección')
    paciente_telefono = fields.Char(compute='_get_telefono', fnct_inv=_set_telefono , type=" Char", store= False,
                            string=u'telefono')
    paciente_nombre_acompaniante = fields.Char(compute='_get_nom_acom', fnct_inv=_set_nom_acom , type=" Char", store= False,
                            string=u'Nombre Acompañante')
    paciente_telefono_acompaniante = fields.Char(compute='_get_tel_acom', fnct_inv=_set_tel_acom , type=" Char", store= False,
                            string=u'Teléfono Acompañante')
    paciente_nombre_responsable = fields.Char(compute='_get_nom_res', fnct_inv=_set_nom_res , type=" Char", store= False,
                            string=u'Nombre responsable')
    paciente_telefono_responsable = fields.Char(compute='_get_tel_res', fnct_inv=_set_tel_res , type=" Char", store= False,
                            string=u'Telefono responsable')
    paciente_ocupacion_id = fields.Many2one(compute='_get_profesion', fnct_inv=_set_profesion , type=" Many2one", store= False,
                            string=u'Profesión', relation='doctor.patient.ocupacion')
    paciente_insurer_prepagada_id = fields.Many2one(compute='_get_insurer', fnct_inv=_set_insurer , type=" Many2one", store= False,
                            string=u'Aseguradora', relation='doctor.insurer')
    paciente_parentesco_id = fields.Many2one(compute='_get_parentesco', fnct_inv=_set_parentesco , type=" Many2one", store= False,
                            string=u'Parentesco', relation='doctor.patient.parentesco')
    diseases_ago_ids = fields.One2many(compute='load_attentions_diseases_ago', relation="doctor.attentions.diseases", type="One2many", store=False, readonly=True, method=True, string=u"Diagnósticos Anteriores")
    paciente_creencias = fields.Char(compute='_get_creencias', fnct_inv=_set_creencias , type=" Char", store= False,
                            string=u'Creencias')
    paciente_blood_type = fields.Selection(compute='_get_blood_type',fnct_inv=_set_blood_type, type='selection', selection=[('A', 'A'), ('B', 'B'), ('AB', 'AB'), ('O', 'O')], string='Tipo Sangre',
                                store=False)
    paciente_rh = fields.Selection(compute='_get_rh',fnct_inv=_set_rh, type='selection', selection=[('+', '+'), ('-', '-'), ], string='Rh',
                                store=False)
...
class doctor_attention_resumen(models.Model):
    ...
    review_systems_id = fields.One2many('doctor.review.systems', 'attentiont_id', 'Revision por Sistema',
                                         readonly=True)
...
class doctor_paraclinical_monitoring(models.Model):
    ...
    seguimientos_id =  fields.Many2one('doctor.name_paraclinical_monitoring', u'Seguimiento Paraclínico',
                                            domain="[('name','!=','CARGAR TODOS')]")
...
class doctor_name_paraclinical_monitoring(models.Model):
...
class doctor_co_schedule_dias_excepciones(models.Model):
...
#Localidades
class doctor_neighborhood(models.Model):
    ...
    state_id =  fields.Many2one('res.country.state', 'Departamento/Provincia', required=True,
                                 domain="[('country_id','=',country_id)]")
    city_id =  fields.Many2one('res.country.state.city', 'Ciudad/Localidad', required=True,
...
#Localidades
class doctor_localidad_country(models.Model):
...
class doctor_co_schedule_inherit(models.Model):
...
class doctor_time_space(models.Model):
...
class doctor_espacios(models.Model):
...
class doctor_patient_note(models.Model):
...
class doctor_list_report(models.Model):
...
class doctor_list_report_print(models.Model):
...
class doctor_otra_prescripcion(models.Model):
...
class doctor_professional(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        for node in doc.xpath("//field[@name='procedures_ids']"):
            ...
            modelo_buscar = self.env['doctor.aseguradora.procedimiento']
            record = modelo_buscar.search([])
            if record:
                for datos in modelo_buscar.browse(record):
...
class doctor_attention_medicamento_otro_elemento(models.Model):
...
class doctor_attentions_recomendaciones(models.Model):
    ...
    tipo_plantilla = fields.Selection(tipo_plantillas, 'Tipo Plantilla',default='01')
...
class doctor_attentions_certificado(models.Model):
...
class multi_imagen(models.Model):
    _name = 'multi_imagen'
...
class doctor_drugs(models.Model):
    ...
    #        drugs_ids = self.search([])
    #        last_id = drugs_ids and max(drugs_ids)
    #        _logger.info('drugssss')
    #        _logger.info('drugssss')
    #        _logger.info('drugssss')
    #        _logger.info('drugssss')
    #        _logger.info(last_id)

    #        code = self.browse(last_id).code
    #        _logger.info(code)
    #        first_code = code[0:2]
    ...
    #        vals['code'] = new_code
    #    _logger.info(vals)
    #    return super(doctor_drugs,self).create(vals)

    def name_get(self):
        if not len(self._ids):
            return []
        reads = self.read(['atc_id', 'pharmaceutical_form', 'drugs_concentration', 'administration_route'])
        res = []
        for record in reads:
...
class doctor_atc(models.Model):
    ...
    def create(self, vals):
        ...
        if self.env['doctor.doctor'].modulo_instalado('l10n_co_doctor'):
            ...
            vals['name'] = atc.upper()
        return super(doctor_atc,self).create(vals)
...
class doctor_invoice_co (models.Model):
    _inherit = "account.invoice"
    _name = "account.invoice"

    #ref = fields.related('patient_id', 'ref', type=" Char", relation="doctor.patient", string="Nº de identificacion",
    #                       required=True, readonly=True),
    tipo_usuario_id =  fields.Many2one('doctor.tipousuario.regimen', 'Tipo usuario', required=False)
...
class doctor_sales_order_co (models.Model):
    _inherit = "sale.order"
    _name = "sale.order"
    ...
    #ref = fields.related('patient_id', 'ref', type=" Char", relation="doctor.patient",
    #                      string="Nº de identificacion", required=True, readonly=False),
    tipo_usuario_id = fields.Many2one('doctor.tipousuario.regimen', 'Tipo usuario', required=False),
    contrato_id = fields.Many2one('doctor.contract.insurer', 'Contrato', required=False),
...
class doctor_configuracion(models.Model):
...
class doctor_parametrizacion(models.Model):
    _name = "doctor.parametrizacion"

    doctor_configuracion_id = fields.Many2one('doctor.configuracion', 'configuracion')
    procedures_id = fields.Many2one('product.product', 'Procedimiento en salud', required=True, ondelete='restrict',
...
class doctor_aseguradora_procedimiento(models.Model):
    _name = "doctor.aseguradora.procedimiento"

    procedures_id = fields.Many2one('product.product', 'Procedimientos en Salud', required=True, ondelete='restrict')
    aseguradora_procedimiento_id =  fields.Many2one('doctor.configuracion_procedimientos_institucion',
                                                     'Procedimientos Institucion')
